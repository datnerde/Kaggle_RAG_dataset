...
#apply xgb model.
xgb_model <- xgboost(data=as.matrix(x_train), label=as.matrix(train_target), booster="gbtree", objective="reg:linear", eta=0.3, max_depth=5, min_child_weight=1
,gamma = 0,subsample = 0.75,colsample_bytree = 1,colsample_bylevel = 1
,lambda = 1,alpha = 0,nrounds = 60,scale_pos_weight = 1)

#submitting the file.
submission <- fread("../input/sample_submission.csv")
submission$loss <- log1p(predict(xgb_model, as.matrix(x_test)))
